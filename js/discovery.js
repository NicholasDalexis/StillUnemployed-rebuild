...
(function(root,factory){var api=factory();if(typeof module==='object'&&module.exports)module.exports=api;else root.SUDiscovery=api.create(root);})(typeof window!=='undefined'?window:this,function(){
  ...
  function create(root){var App,first=true,started=false,account=null,showHidden=false,form=false,draft=null,undo=null,message='',visitReady=false,recommendations=[],recOpen=false,promptDismissed=false,preferenceResult='',formError='';
    ...
    function html(esc){var d=data(),hasHidden=Object.keys(d).some(function(k){return k.indexOf('job:')===0&&d[k]&&d[k].hidden;});
      if(!signed()&&!hasHidden&&!message)return '';
      var out='<section class="su-discovery" aria-label="Your job preferences"><div class="su-discovery-tools">'+(signed()?'<button type="button" id="su-preferences-open" data-discovery="settings" aria-haspopup="dialog">Your preferences</button>':'')+(hasHidden?'<button type="button" data-discovery="hidden" aria-pressed="'+showHidden+'">'+(showHidden?'Hide dismissed jobs':'Show hidden jobs')+'</button>':'')+'</div>';
      if(message||preferenceResult)out+='<p role="status"><span id="su-preference-status">'+esc(resultText())+'</span>'+(preferenceResult?' <button type="button" id="su-preference-retry" data-discovery="retry"'+(root.SUAuth.syncState&&root.SUAuth.syncState()==='error'?'':' hidden')+'>Retry sync</button>':'')+(undo?' <button type="button" data-discovery="undo">Undo</button>':'')+'</p>';
      if(recommendations.length&&!preferencesOpen()&&!blocked()){
    ...
    function start(app){App=app;if(started)return;started=true;try{var legacy=JSON.parse(root.localStorage.getItem('su_reported_links')||'[]'),guest=JSON.parse(root.localStorage.getItem('su_discovery_guest')||'{}');legacy.forEach(function(link){if(!guest[key(link)])guest[key(link)]={link:link,reason:'unavailable',hidden:true,confirmed:false};});root.localStorage.setItem('su_discovery_guest',JSON.stringify(guest));}catch(e){}try{first=!root.localStorage.getItem('su_discovery_started');root.localStorage.setItem('su_discovery_started','1');}catch(e){}
      function auth(){account=root.SUStore&&root.SUStore.owner();form=false;draft=null;undo=null;message='';formError='';preferenceResult='';promptDismissed=false;showHidden=false;recommendations=[];recOpen=false;visitReady=false;rememberVisit();render();}
      ...
      root.document.addEventListener('submit',function(e){if(e.target.id!=='su-discovery-form')return;e.preventDefault();if(!signed())return;try{if(!write('profile',readForm()))throw Error('Not saved');write('promptAnswered',true);form=false;draft=null;promptDismissed=true;preferenceResult='saved';message='';track('preference_save');render();}catch(err){formError='Could not save preferences. Your answers are still here. Please try again.';render();}});
      root.document.addEventListener('click',function(e){var button=e.target.closest&&e.target.closest('[data-discovery]');if(!button)return;e.preventDefault();var action=button.getAttribute('data-discovery');if(!signed()&&['hidden','undo','restore'].indexOf(action)<0)return;try{
        if(action==='settings'){form=true;draft=null;message='';formError='';preferenceResult='';}
        ...
        if(action==='settings'&&App.renderOverlays)App.renderOverlays();else render();if(action==='settings'){var panel=root.document.getElementById('su-discovery-form');if(panel){panel.querySelector('input').focus({preventScroll:true});}}
      }catch(err){message='This change could not be saved. Please try again.';formError=message;render();}});
      account=root.SUStore&&root.SUStore.owner();rememberVisit();
    }
    return {start:start,order:order,html:html,preferencesOpen:preferencesOpen,modalHTML:modalHTML,refreshDialog:refreshDialog,closePreferences:closePreferences,dialogOwner:function(){return account;},hidden:hidden,showHidden:function(){return showHidden;},dismiss:dismiss,key:key,confirmedCount:count,profile:useProfile};
  }
