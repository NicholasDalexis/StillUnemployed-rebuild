    ...
    render: function () {
      ...
      out += '<div style="max-width: 1240px; margin: 0 auto; padding: 26px 40px 0; position: relative; height: 100px; box-sizing: border-box;">' +
        ...
        '<div style="position: absolute; top: 22px; left: 50%; transform: translateX(-50%); display: flex; align-items: center; gap: 18px;">' +
          ...
          '<a href="./tracker.html" class="postit trk-nav-a r3">Tracker</a>' +
          '<a href="https://jobhuntrecipe.com/p/jobs-ghost-more-than-hinge-issue-1" target="_blank" rel="noopener" class="postit trk-nav-a r4">Advice</a>' +
        '</div>' +
      '</div>';
      ...
      out += '<div class="trk-form">' +
        '<div class="trk-form-label">applied somewhere else? log it ↓</div>' +
      ...
      // ---- rows ----
      if (!this.rows.length) {
        out += '<div class="trk-empty">' +
          '<div class="trk-empty-title">nothing logged yet</div>' +
          '<div class="trk-empty-hint">tap "I applied!" on any job on the board and it shows up here on its own — or add one by hand above</div>' +
        '</div>';
      } else {
        ...
        this.rows.forEach(function (r) {
          ...
          var linkHtml = r.link
            ? '<a class="trk-linka" href="' + esc(r.link) + '" target="_blank" rel="noopener">posting ↗</a>'
            : '';
          ...
          out += '<div class="trk-row' + (del ? ' deleting' : '') + '" data-id="' + esc(r.id) + '">' +
            '<div class="trk-main">' +
              ...
              '<div class="trk-src">' + (r.source === 'StillUnemployed' ? 'via the board' : 'added by you') + '</div>' +
            '</div>' +
            ...
            '<input class="trk-notes" data-id="' + esc(r.id) + '" value="' + esc(r.notes || '') + '" placeholder="notes... (recruiter name, next step)">' +
