...
const source = fs.readFileSync(path.join(__dirname, '../../js/auth.js'), 'utf8').replace(/[A-Za-z0-9_.+%-]+@gmail\.com/gi, 'qa@example.invalid');
...
async function authUI(hostname = 'preview--stillunemployed.netlify.app', options = {}) {
  ...
  // Substitute SDK loading after the reserved QA identity above. The hostname gate, auth callback and
...
test('auth shows Sign in when signed out and Account when authenticated', async () => {
  ...
  ui.signIn(null);assert.equal(ui.label.textContent,'Sign in');assert.equal(ui.attrs['aria-label'],'Sign in with Google');assert.equal(ui.button.dataset.state,'signed-out');assert.equal(ui.button.disabled,false);
...
test('both public production hostnames initialize Google authentication', async () => {
  for (const hostname of ['stillunemployed.com', 'www.stillunemployed.com']) {
    const ui = await authUI(hostname);assert.equal(ui.imports, 3);ui.signIn(null);assert.equal(ui.label.textContent, 'Sign in');
  }
});
...
test('resolved Firebase QA identity suppresses measurement while hints remain ordinary readable help',async()=>{
 ...
 ui.signIn({uid:'qa',email:'qa@example.invalid',emailVerified:false});assert.equal(ui.api.measurementExcluded(),false);assert.equal(ui.api.qaAdmin(),false);assert.equal(ui.qaLabel.hidden,true);
 ui.signIn({uid:'qa',email:'QA@Example.invalid',emailVerified:true});assert.equal(ui.api.measurementExcluded(),true);assert.match(ui.attrs['data-su-help'],/QA admin: your testing is excluded/);assert.equal(ui.api.qaAdmin(),true);assert.equal(ui.qaLabel.hidden,false);assert.match(ui.attrs['aria-label'],/QA admin/);
...
test('QA label disappears immediately while another tab changes ownership',async()=>{
 const ui=await authUI();ui.signIn({uid:'qa',email:'qa@example.invalid',emailVerified:true});assert.equal(ui.api.qaAdmin(),true);
...
test('confirmed Google identity can request a dashboard token when the page has no job cache',async()=>{
 ...
 ui.signIn({uid:'qa',email:'qa@example.invalid',emailVerified:true,getIdToken:async()=>'synthetic-token'});
