...
test('starter six keep the intended lanes, highest annual minimum, with no duplicates',()=>{
 ...
 const ranked=D.starter(jobs,P);assert.deepEqual(ranked.slice(0,6).map(D.lane),['marketing','graphic','product','photography','social','copy']);assert.equal(ranked[0].link,jobs[2].link);assert.equal(new Set(ranked.map(j=>j.link)).size,jobs.length);assert.deepEqual(D.starter(jobs.slice().reverse(),P).map(j=>j.link),ranked.map(j=>j.link));
});
test('missing lanes and salary do not invent cards or annualize hourly pay',()=>{const jobs=[job('Social',1,'$60/hr'),job('Social',2,'$100K'),job('Fashion Design',3,'Not disclosed')];assert.deepEqual(D.starter(jobs,P).map(j=>j.link),[jobs[1].link,jobs[0].link,jobs[2].link]);});
