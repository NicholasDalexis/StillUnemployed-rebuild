...
test('HTTP collection survives store restart and feeds private aggregate plus account ranking',async()=>{
  ...
  await new Promise(resolve=>server.listen(0,'127.0.0.1',resolve));const origin='http://127.0.0.1:'+server.address().port;d.env.SU_ALLOWED_ORIGINS='https://stillunemployed.com';
  const headers={Origin:'https://stillunemployed.com',Authorization:'Bearer alice','Content-Type':'application/json'};
  try{
