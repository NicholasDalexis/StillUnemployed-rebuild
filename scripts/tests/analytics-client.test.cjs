...
function harness(local=storage(),session=storage(),responses=[],gpc=false,hostname='stillunemployed.com'){
...
test('preview and local browsing never create measurement identifiers or network events',async()=>{for(const host of ['preview--stillunemployed.netlify.app','abc--stillunemployed.netlify.app','localhost']){const h=harness(storage({su_consent_v3:'granted',su_personalization_v1:'granted'}),storage(),[],false,host);h.start();h.auth(true);h.api.emit('page_view',{});h.tick(60000);await h.api.flush();assert.equal(h.requests.length,0);assert.equal(h.local.getItem('su_analytics_visitor'),null);assert.equal(h.api.excluded(),true);}});

test('successful history reset clears device recent history, but a server failure never claims full reset',async()=>{for(const ok of [true,false]){const h=harness(storage(),storage(),[{ok,status:ok?200:503}]);let cleared=0;h.window.SUBoardExperience={clearHistory:()=>cleared++};h.auth(true);if(ok){await h.api.reset();assert.equal(cleared,1);}else{await assert.rejects(()=>h.api.reset());assert.equal(cleared,0);}}});

test('history reset cannot clear another account after an asynchronous token crosses account changes',async()=>{const h=harness();let release,cleared=0;h.auth(true);h.window.SUAuth.getToken=()=>new Promise(resolve=>{release=resolve;});h.window.SUBoardExperience={clearHistory:()=>cleared++};const pending=h.api.reset();h.auth(false,true);release('old-account-token');await assert.rejects(()=>pending,/Account changed/);assert.equal(cleared,0);assert.equal(h.requests.length,0);});
test('newsletter receipt request sends only consented canonical job context, no raw link or email',async()=>{const h=harness(storage({su_consent_v3:'granted'}));const job={link:'https://example.invalid/job',co:'Example',role:'Designer'};await h.api.registerJobs([job]);await h.api.newsletterRequest('a'.repeat(64),{placement:'job-detail',cta:'N01',link:job.link,email:'private@example.invalid'});const sent=JSON.parse(h.requests[0].options.body);assert.match(sent.jobId,/^[a-f0-9]{64}$/);assert.equal(sent.cta,'N01');assert.deepEqual(sent.consent,{analytics:true});assert.doesNotMatch(h.requests[0].options.body,/example.invalid|private/);});
test('account change while newsletter revocation is pending cannot reset the new account',async()=>{const h=harness(storage({su_consent_v3:'granted'}));h.auth(true);let release;h.window.SUNewsletter={revoke:()=>new Promise(r=>release=r)};const resetting=h.api.reset();h.auth(false,true);release();await assert.rejects(resetting,/Account changed/);assert.equal(h.requests.filter(r=>r.options.method==='DELETE').length,0);});
