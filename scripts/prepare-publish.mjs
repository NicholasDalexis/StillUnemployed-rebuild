...
export function prepare(root){const dest=join(root,'dist');rmSync(dest,{recursive:true,force:true});mkdirSync(dest,{recursive:true});
  for(const name of readdirSync(root)){
    const publicFile=['index.html','jobs.html','internships.html','tracker.html','suggest.html','privacy.html','terms.html','versions.html','analytics.html','style-guide.html','404.html'].includes(name)||['jobs-data.json','internships-data.json','releases.json','robots.txt','sitemap.xml','favicon.ico','_redirects','_headers','site.webmanifest'].includes(name);
    ...
    cpSync(source,join(dest,name),{recursive:true,filter:p=>{const stat=lstatSync(p);if(stat.isSymbolicLink())throw Error('Public symlink rejected');return !p.includes('/assets/og-src/');}});
  }
  return dest;
}
